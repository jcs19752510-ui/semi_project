...
class PreprocessCheckResponse(BaseModel):
    target_distribution: dict          # {"satisfied": 73012, "unsatisfied": 3008} — 2026-08-24 실측 정정(72928은 오기, 76020-3008=73012)
...
class ModelResultResponse(BaseModel):
    curves: list[dict]  # [{"model": "lightgbm", "label": "LightGBM", "auc": 0.8264, "fpr": [...], "tpr": [...]}] — 2026-08-24 실측(v2는 4개 피처만 사용해 원본 노트북 수치와 다름)
```
...
sequenceDiagram
    ...
    B-->>U: "전처리 데이터 검증결과" 4개 차트 렌더링<br/>"모델 수행결과" ROC 차트 렌더링(AUC=0.8264 표시, 2026-08-24 실측)
```
...
| AC-DV5 | ROC 차트의 AUC 값은 곡선 아래 면적을 실제 계산한 값과 일치해야 하며, 모델을 바꾸면 다른 AUC가 표시되어야 한다 | ✅ 2026-08-24 실측 완료: lightGBM AUC=0.8264, RandomForest AUC=0.8296 — 모델별 상이한 값 확인(둘 다 v2 스코프 4개 피처 기준) |
...
| TC-DV8(브라우저 실측) | 실제 76,020행 데이터로 업무="1.산탄데르", 모델="1.lightGBM" 선택 후 전처리조회 클릭 | ✅ 2026-08-24 완료: TARGET 분포 만족 **73,012**(원안 72,928은 오기) / 불만족 3,008 정확히 표시, ROC 차트 **AUC=0.8264** 표시 — Render 운영 서버 API 실측 + 로컬 브라우저 사용자 확인 | AC-DV3, AC-DV4, AC-DV5 |
...
## 9. 구현 완료 갱신 (2026-08-24)

본 TRD 착수 이후 실제 구현·내부테스트·운영 배포까지 완료했다. 상세 근거는 `정찬성/95.작업결과문서/`의 내부테스트 결과서 4건(1~4차, 타임스탬프 파일명) 참고.

| 항목 | 결과 |
|---|---|
| 백엔드 구현 | `registry.py`/`service.py`/`ml.py` 신규, `crud.py`/`schemas.py`/`router.py` 확장 — §1~§4 스펙대로 완료 |
| 프론트 구현 | `index.html`/`app.js`/`style.css` 전면 교체 — §1 화면 레이아웃대로 완료 |
| 내부테스트 | pytest 21/21 PASS(v1 회귀 9 + v2 신규 12), 실제 76,020행 CSV 기준 실측 완료 |
| §0-1 8개 항목(가이드요청서) | 전항목 확정 완료 — 상세는 가이드요청서 §0-1 각 항목 하단 참고 |
| boxplot 시각화 | whisker를 min/max가 아닌 1.5×IQR 표준방식으로 렌더링하도록 수정(원본 통계치는 유지) |
| 모델 | lightgbm(AUC 0.8264) + RandomForest(AUC 0.8296) 2종 등록, AC-DV5 완전 검증 |
| 운영 배포 | `https://semi-project.onrender.com/dataviz` — Render Starter 플랜(유료)으로 전환해 상시 구동. 배포 초기 500 에러는 로컬 `.env` 부재가 원인(운영 서버 자체는 무관), 이후 정상 확인 |
| 남은 과제 | Neon 등 실제 PostgreSQL 연결(§8 기존 제안과 별개, 인증/DB 미도입 상태 지속), RandomForest 외 3번째 모델 확장 시 §0-1-4 재검토 |
